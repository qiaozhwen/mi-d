...
export default class extends React.Component {
  ...
  loadTypeData({ latitude, longitude }) {
    fetch(
      `/shopping/openapi/entries?latitude=${latitude}&longitude=${longitude}&templates[]=main_template&templates[]=favourable_template&templates[]=svip_template`,
    )
      .then((res) => {
        if (res.status === 200) {
          res.json().then((data) => {
  ...
  loadRestaurantData({ latitude, longitude }, rank_id = '') {
    fetch(
      `/shopping/v3/restaurants?latitude=${latitude}&longitude=${longitude}&offset=${this.rests.length}limit=8&extras[]=activities&extras[]=tags&extra_filters=home&rank_id=&terminal=h5&rank_id=${rank_id}`,
    )
      .then((res) => {
        if (res.status === 200) {
          res.json().then((data) => {
        ...
        <div className={styles.info}>
          ...
          {data.recommend && data.recommend.reason ? (
            <div
              ...
              style={{ color: data.recommend.color }}
            >
              <div className={styles.logo}>
